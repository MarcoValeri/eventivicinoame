...
func EventsGetThemByPeriodOfTimeWithoutYear(getMonth, getLimit int) ([]EventWithRelatedFields, error) {
	...
	mySqlQuery += "WHERE"
	mySqlQuery += " "
	mySqlQuery += "((? BETWEEN MONTH(events.event_start_date) AND MONTH(events.event_end_date)) OR (MONTH(events.event_start_date) > MONTH(events.event_end_date) AND (? >= MONTH(events.event_start_date) OR ? <= MONTH(events.event_end_date))))"
	mySqlQuery += " "
